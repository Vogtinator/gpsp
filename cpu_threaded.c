...
#define check_pc_region(pc)                                                   \
  ...
  {                                                                           \
    ...
    pc_address_block = memory_map_read(pc_region);                            \
                                                                              \
    if((s32)pc_address_block < 0)                                             \
      pc_address_block = load_gamepak_page(pc_region & 0x3FF);                \
	else                                                                      \
	  pc_address_block += pc_region * 0x8000;                                 \
  }                                                                           \
...
#define index_to_tag(index) ((index) + ((index) & ~0x7F) + 0x8080)
#define tag_to_index(tag) (((tag) & 0x7F) | (((tag) & 0x7F00) >> 1))

u8 *ram_block_ptrs[1024 * 64];
u32 ram_block_tag_top = 0;

u8 *bios_block_ptrs[1024 * 8];
u32 bios_block_tag_top = 0;
...
#define fill_tag_arm(mem_type)                                                \
  location[0] = index_to_tag(mem_type##_block_tag_top);                       \
  location[1] = 0xFFFF                                                        \

#define fill_tag_thumb(mem_type)                                              \
  *location = index_to_tag(mem_type##_block_tag_top)                          \
...
#define block_lookup_translate(instruction_type, mem_type, smc_enable)        \
  block_tag = *location;                                                      \
  if(!(block_tag & 0x8080) || (block_tag == 0xFFFF))                          \
  {                                                                           \
    __label__ redo;                                                           \
  ...
  {                                                                           \
    block_address = mem_type##_block_ptrs[tag_to_index(block_tag)];           \
  }                                                                           \
