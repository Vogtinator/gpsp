...
u32 load_backup(char *name)
{
  ...
  {
    u32 backup_size = file_length((u8*)name, backup_file);
...
void update_backup()
{
  ...
  {
#ifdef NSPIRE_BUILD
	save_backup_wrapper(backup_filename);
#else
...
#ifdef NSPIRE_BUILD
#define CONFIG_FILENAME "game_config.txt.tns"
#else
...
u32 load_gamepak(char *name)
{
  ...
  if(!strcmp(dot_position-4, ".zip.tns"))
...
#ifdef NSPIRE_BUILD
    change_ext(gamepak_filename, backup_filename, ".sav.tns");
#else
...
#ifdef NSPIRE_BUILD
    change_ext(gamepak_filename, cheats_filename, ".cht.tns");
#else
...
#define dma_vars_gamepak(type)                                                \
  ...
  {                                                                           \
    ...
    type##_address_block += type##_current_region * 0x8000;                   \
...
#define map_handlers(type, start, end, name)                                  \
  for(map_offset = (start) / 0x8000; map_offset < ((end) / 0x8000);           \
   map_offset++)                                                              \
  {                                                                           \
    memory_map_##type(map_offset) = type##_handlers_##name + 0x80000000;      \
  }                                                                           \

#define map_null(type, start, end)                                            \
  map_handlers(type, start, end, ext)                                         \
...
#define map_vram(type)                                                        \
  for(map_offset = 0x6000000 / 0x8000; map_offset < (0x7000000 / 0x8000);     \
   map_offset += 4)                                                           \
  {                                                                           \
    memory_map_##type(map_offset) = vram - map_offset * 0x8000;               \
    memory_map_##type(map_offset + 1) = vram + 0x8000 * (1 - (map_offset+1)); \
    memory_map_##type(map_offset + 2) = vram + 0x8000 * (2 - (map_offset+2)); \
    memory_map_##type(map_offset + 3) = vram + 0x8000 * (2 - (map_offset+3)); \
  }                                                                           \
...
#define map_vram_firstpage(type)                                              \
  ...
  {                                                                           \
    memory_map_##type(map_offset) = vram - map_offset * 0x8000;               \
    memory_map_##type(map_offset + 1) = type##_handlers_ext + 0x80000000;     \
    memory_map_##type(map_offset + 2) = type##_handlers_ext + 0x80000000;     \
    memory_map_##type(map_offset + 3) = type##_handlers_ext + 0x80000000;     \
  }                                                                           \
...
u32 evict_gamepak_page()
{
  ...
  memory_map_read((0x8000000 / (32 * 1024)) + physical_index) = read_handlers_ext + 0x80000000;
  memory_map_read((0xA000000 / (32 * 1024)) + physical_index) = read_handlers_ext + 0x80000000;
  memory_map_read((0xC000000 / (32 * 1024)) + physical_index) = read_handlers_ext + 0x80000000;
...
u8 *load_gamepak_page(u32 physical_index)
{
  ...
  u32 region = (0x08000000 / (32 * 1024)) + physical_index;
  memory_map_read(region) = swap_location - region * 0x8000;
  region += (0x02000000 / (32 * 1024));
  memory_map_read(region) = swap_location - region * 0x8000;
  region += (0x02000000 / (32 * 1024));
  memory_map_read(region) = swap_location - region * 0x8000;
...
void init_memory()
{
  ...
  map_null(read, 0x10000000, 0xFFFF8000);
  memory_map_read(0xFFFF8000 / 0x8000) = read_handlers_ext + 0x80000000;
  ...
  {
    map_handlers(write, 0x6000000, 0x7000000, vram);
  }

  map_null(write, 0x7000000, 0x8000000);
  map_null(write, 0x8000000, 0xE000000);
  map_null(write, 0xE000000, 0x10000000);
  map_null(write, 0x10000000, 0xFFFF8000);
  memory_map_write(0xFFFF8000 / 0x8000) = write_handlers_ext + 0x80000000;
...
{
  memory_map_read(0) = bios_rom - 0 * 0x8000;
}

void bios_region_read_protect()
{
#if defined(GP2X_BUILD) || defined(NSPIRE_BUILD)
  memory_map_read(0) = read_handlers_ext + 0x80000000;
#endif
}
