...
#define check_pc_region()                                                     \
  ...
  {                                                                           \
    ...
    if((s32)pc_address_block < 0)                                             \
	  ...
	  pc_address_block += new_pc_region * 0x8000;                             \
  }                                                                           \
...
{                                                                             \
  ...
  if(((_address & aligned_address_mask##size) == 0) &&                        \
   (s32)(map = memory_map_read(_address >> 15)) >= 0)                         \
  {                                                                           \
    dest = *((type *)((u8 *)map + _address));                                 \
  }                                                                           \
  else                                                                        \
  {                                                                           \
...
{                                                                             \
  ...
  {                                                                           \
    ...
    memory_writes_##type++;                                                   \
                                                                              ...
                                                                              \
  if(((_address & aligned_address_mask##size) == 0) &&                        \
   (s32)(map = memory_map_write(_address >> 15)) >= 0)                        \
  {                                                                           \
    *((type *)((u8 *)map + _address)) = value;                                \
  }                                                                           \
  else                                                                        \
  {                                                                           \
...
u32 function_cc step_debug(u32 pc, u32 cycles)
{
   ...
   (current_debug_state == STEP_RUN))
  {
    ...
    printf("%x instructions in, VCOUNT %d, cycles remaining: %d \n",
     (unsigned int)instruction_count, io_registers[REG_VCOUNT], (int)cycles);
    ...
    switch(key)
    {
      ...
      case 'a':
      {
        ...
        save_state((char*)current_savestate_filename, current_screen);
        free(current_screen);
        break;
      }
