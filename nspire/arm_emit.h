...
void function_cc execute_load_u32_safe(u32 address);
...
#define arm_block_memory_load()                                               \
  generate_function_call(execute_load_u32_safe);                              \
  write32((pc + 8));                                                          \
  generate_store_reg(reg_a1, i)                                               \
...
#define arm_block_memory_offset_down_a()                                      \
  generate_sub_imm(reg_a0, ((word_bit_count(reg_list) * 4) - 4), 0)           \
...
#define arm_block_memory_offset_up()                                          \
  generate_add_imm(reg_a0, 4, 0)                                              \

#define arm_block_memory_writeback_down()                                     \
  generate_sub_imm(i, (word_bit_count(reg_list) * 4), 0);                     \
  generate_store_reg(i, rn)                                                   \

#define arm_block_memory_writeback_up()                                       \
  generate_add_imm(i, (word_bit_count(reg_list) * 4), 0);                     \
  generate_store_reg(i, rn)                                                   \
...
{                                                                             \
  ...
  u32 i;                                                                      \
                                                                              \
  i = prepare_load_reg(reg_a1, rn);                                           \
  ARM_BIC_REG_IMM(0, reg_a0, i, 0x03, 0);                                     \
...
{                                                                             \
  thumb_decode_hireg_op();                                                    \
  u32 _rd = prepare_store_reg(reg_rd, rd);                                    \
  u32 _rs = prepare_load_reg_pc(_rd, rs, 4);                                  \
  if (_rd == reg_rd)                                                          \
  {                                                                           \
    _rd = _rs;                                                                \
  }                                                                           \
...
#define thumb_block_address_preadjust_up()                                    \
  generate_add_imm(reg_a0, (bit_count[reg_list] * 4), 0)                      \
...
#define thumb_block_address_preadjust_push_lr()                               \
  generate_sub_imm(reg_a0, ((bit_count[reg_list] + 1) * 4), 0)                \
...
#define thumb_block_address_postadjust_up(base_reg)                           \
  generate_add_reg_reg_imm(reg_a1, reg_a0, (bit_count[reg_list] * 4), 0);     \
  generate_store_reg(reg_a1, base_reg)                                        \

#define thumb_block_address_postadjust_down(base_reg)                         \
  generate_sub_reg_reg_imm(reg_a1, reg_a0, (bit_count[reg_list] * 4), 0);     \
  generate_store_reg(reg_a1, base_reg)                                        \

#define thumb_block_address_postadjust_pop_pc(base_reg)                       \
  generate_add_reg_reg_imm(reg_a1, reg_a0,                                    \
...
{                                                                             \
  ...
  u32 i;                                                                      \
                                                                              \
  i = prepare_load_reg(reg_a0, base_reg);                                     \
  ARM_BIC_REG_IMM(0, reg_a0, i, 0x03, 0);                                     \
