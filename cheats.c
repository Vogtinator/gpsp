...
int scanhex(u8 *str, u32 *address_ptr, u32 *value_ptr)
{
	while(isspace(*str))
		str++;
	if(!isxdigit(*str))
...
void add_cheats(u8 *cheats_filename)
{
  ...
  if(cheats_file)
  {
    while(num_cheats < MAX_CHEATS && fgets(current_line, 256, cheats_file))
    {
      ...
      if(current_cheat_variant != CHEAT_TYPE_INVALID)
      {
        ...
        {
          if(!scanhex(current_line, &address, &value))
			break;
